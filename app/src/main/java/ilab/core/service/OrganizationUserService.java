...
public class OrganizationUserService {
	...
	public OrganizationUser addMemberToOrg(User userToAdd, Authentication auth) throws Exception {
		...
		if (org.getOwner() == userInvite && org.getStatus() == OrganizationStatus.ACTIVE) {
			...
			orgUserRepo.save(tempOrgUser);
			OrganizationUser orgUser = orgUserRepo.findByUser(userToAdd).orElseThrow();
			sendInvitation(userToAdd.getEmail(), orgUser.getId());
			return tempOrgUser;
		} else {
	...
	public void deleteMemberFromOrg(User userAdded, Authentication auth) {
		OrganizationUser orgUser = orgUserRepo.findByUser(userAdded).orElseThrow();
